...
<template>
    <div>
        <yd-navbar slot="navbar" title="耗卡记录">
                ...
                <yd-timeline class="demo-small-pitch">
                    <yd-timeline-item v-for="(item, index) in orderData" :key="index">
                        <div>
                            <b>耗卡项目</b>:
                            <ul>
                                <li v-for="(it, index) in item.items_info" :key="index">
                                    项目名称: {{ it.item_name }} &nbsp;
                                    耗卡次数: {{ it.use_time }}
                                </li>
                            </ul>
                        </div>
                        <p><b>消耗金额</b>: {{ item.use_money }}</p>
                        ...
                        <p style="margin-top: 10px;"><b>耗卡时间</b>:{{ item.add_time }}</p>
                    </yd-timeline-item>
                </yd-timeline>
            </yd-list>
             <!--数据全部加载完毕显示-->
...
<script>
    ...
    import { getUseOrderList } from '../api/api'
    export default {
        data () {
            return {
                orderData: [],
