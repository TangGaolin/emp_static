...
import { login,logout,getEmployeeList } from '../api/api'
...
export const getEmpListAction = ({commit}, params) => {
    return new Promise((resolve, reject)=> {
        getEmployeeList(params).then((response) => {
            if(0 !== response.statusCode) {
                ...
                commit(types.SET_EMPS, response.data); //获得的数据通过mutation，存入store中
                resolve(response)
            }
        }).catch((error) => {
            console.log(error)
